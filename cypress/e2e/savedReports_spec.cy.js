describe('Saved Reports', () => {
  ...
  it('Should view saved reports allowing a user to delete one and navigate back to /match', () => {
    ...
    .get('.saved-report').first().find('p').first().should('have.text', 'Lady Young')
    .get('.saved-report').first().find('.saved-score').last().should('have.text', '73%')
    .get('.saved-report').first().find('p').last().should('have.text', 'Levi Ackerman')
    ...
    .get('.saved-report').first().find('p').first().should('have.text', 'Lady Young')
    .get('.saved-report').first().find('.saved-score').last().should('have.text', '22%')
    .get('.saved-report').first().find('p').last().should('have.text', 'Leorio Paradinight')
    .get('.classic-button').last().click()
    cy.location('pathname').should('eq', '/match')
    .get('h1').should('have.text', 'Pick Your Match')
  })
  it('Should allow the user to navigate back to see their saved report from the saved reports page', () => {
    cy.visit('http://localhost:3000/savedreports')
    cy.wait('@getAllSaved')
    .get('.view-report-btn').first().click()
    cy.location('pathname').should('eq', '/savedreports/1691015485484')
    cy.get('h1').should('have.text', 'Compatibility Results')
    .get('.whole-report-wrapper').find('.saved-checkmark')
